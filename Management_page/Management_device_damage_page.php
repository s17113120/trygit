...
<body>
        ...
        <div class="collapse navbar-collapse" id="navbarSupportedContent">
            <ul class="navbar-nav mr-auto">
                ...
                <div class="dropdown" onmouseover="document.getElementById('deviceDropDownMeun').style.display = 'block';" onmouseout="document.getElementById('deviceDropDownMeun').style.display = 'none';">
                    ...
                    <div class="dropdown-menu dDropMenu" id="deviceDropDownMeun" aria-labelledby="deviceMenuButton" style="background-color: rgba(255, 202, 229, 0.9); display: none;" onmouseout="document.getElementById('deviceDropDownMeun').style.display = 'none';">
                        ...
                        <a class="dropdown-item active" href="../Management_page/Management_device_damage_page.php">毀損</a>
    ...
    <div class="container">
        <div class="row mt-3">
            <div class="col">
                <div class="alert alert-danger" style="width:180px" role="alert">
                    <i class="fas fa-arrow-alt-circle-down"></i>　設備毀損
                </div>
            </div>
            <div class="col">
...
<script src="https://stackpath.bootstrapcdn.com/bootstrap/4.1.1/js/bootstrap.min.js" integrity="sha384-smHYKdLADwkXOn1EmN1qk/HfnUcbVRZyYmZ4qpPea6sjB/pTJ0euyQp0Mk8ck+5T" crossorigin="anonymous"></script>
    ...
    <script>
        ...
        function showDeviceDamageTable(){
            $.ajax({
                url: '../Management_all_api/Management_device_damage_api.php',
                method: 'post',
                dataType: 'json',
                data: {
                    type: "showdevicedamagedata",
                }
            }).done( res => {



                let allDamageData = res.toshowdata;
                let onePageHowData = 2;
                let howPage = Math.ceil(allDamageData.length / onePageHowData);
                ...
                if (allDamageData.length > 0) {

                    let allDamageDataStr = "";
                    for (let i=0 ; i<allDamageData.length ; i++) {
                        allDamageDataStr +=
                        `<tr data-id="${allDamageData[i].D_Id}" data-page="${Math.ceil((i+1)/onePageHowData)}">
                            <td>
                                <!-- update Button -->
                                <button type="button" class="btn btn-outline-warning" data-toggle="modal" data-target="#updateModalCenter" id="${allDamageData[i].D_Id}" onclick="updateFunc(this)"><i class="fas fa-pencil-alt"></i></button>

                            </td>
                            <td data-type="D_Id">
                                ${allDamageData[i].D_Id}
                            </td>
                            <td data-type="D_Number">
                                ${allDamageData[i].D_Number}
                            </td>
                            <td data-type="D_Name">
                                ${allDamageData[i].D_Name}
                            </td>
                            <td data-type="D_Model">
                                ${allDamageData[i].D_Model}
                            </td>
                            <td data-type="D_Day">
                                ${allDamageData[i].D_Day}
                            </td>
                            <td data-type="D_Unit">
                                ${allDamageData[i].D_Unit}
                            </td>
                            <td data-type="D_Details">
                                ${allDamageData[i].D_Details}
                            </td>
                            <td data-type="D_Lend">
                                ${(allDamageData[i].D_Status == 1 ? "毀損" : "怪怪的")}
                            </td>
                        </tr>`;
                    }

                    document.querySelector("#deviceShowTable").innerHTML = allDamageDataStr;
                    ...
                    if (searchData.length > 0) {
                        for (let i=0 ; i<searchData.length ; i++) {
                            searchDataStr +=
                                ...
                                <td data-type="D_Lend">
                                    ${(searchData[i].D_Status == 1 ? "毀損" : "怪怪的")}
                                </td>
                            </tr>`;
                        }
